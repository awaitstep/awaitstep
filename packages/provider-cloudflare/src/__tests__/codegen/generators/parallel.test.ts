...
describe('generateParallel', () => {
  it('generates Promise.all with branches', () => {
    const ir = makeIR([
      { id: 'a', type: 'step', name: 'Task A', position: { x: 0, y: 0 }, version: V, provider: P, data: { code: 'return "a";' } },
      { id: 'b', type: 'step', name: 'Task B', position: { x: 0, y: 0 }, version: V, provider: P, data: { code: 'return "b";' } },
    ])
  ...
  it('handles single branch', () => {
    const ir = makeIR([
      { id: 'a', type: 'step', name: 'Only task', position: { x: 0, y: 0 }, version: V, provider: P, data: { code: 'return 1;' } },
    ])
    const node = ir.nodes[0]!
    const code = generateParallel(node, ir, generateNodeCode)
    expect(code).toContain('Promise.all')
  ...
  it('returns comment for no branches', () => {
    const ir: WorkflowIR = {
      metadata: { name: 'test', version: 1, createdAt: '', updatedAt: '' },
      nodes: [{ id: 'parallel-1', type: 'parallel', name: 'Empty', position: { x: 0, y: 0 }, version: V, provider: P, data: {} }],
      edges: [],
      entryNodeId: 'parallel-1',
    }
    const node = ir.nodes[0]!
