...
export default function OrgMenu() {
    ...
    const { activeOrganizationId, activeOrg, orgs } = useOrgStore(useShallow((s) => ({
    ...
    return (
        <Popover.Root open={orgOpen} onOpenChange={setOrgOpen}>
            <Popover.Trigger asChild>
                ...
                >
                    <Building2 size={16} />
                    {activeOrg ? <span className="max-w-[120px] truncate text-xs font-medium">{activeOrg?.name}</span> : <Loader2 size={16} className="animate-spin" />}
                    <ChevronDown size={12} />
                </button>
            </Popover.Trigger>
            <Popover.Portal>
