    ...
    <section className="rounded-md border border-border p-4">
      ...
      {isLoading ? (
        ...
        <div className="mt-3 rounded-md border border-dashed border-border py-6 text-center">
          ...
          <p className="mt-0.5 text-[10px] text-muted-foreground/60">Create a key for programmatic access</p>
        </div>
      ) : (
        <div className="mt-3 space-y-1.5">
          {activeKeys.map((key) => {
              ...
              <div key={key.id} className="flex items-center justify-between rounded-md border border-border px-3 py-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <code className="rounded bg-muted/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">{key.keyPrefix}...</code>
                  </div>
                  <div className="mt-0.5 flex items-center gap-1.5">
                    {scopes.map((s) => (
                      <span key={s} className="rounded bg-muted/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">{s}</span>
                    ))}
                ...
                <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setRevokeTarget(key.id)}>Revoke</Button>
              </div>
            )
          })}
        </div>
      ...
      <CreateKeyDialog open={createOpen} onOpenChange={setCreateOpen} onCreated={handleKeyCreated} />
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-md border border-border bg-card p-6 shadow-lg">
          ...
          <Dialog.Description className="mt-1 text-sm text-muted-foreground">Give the key a name and select its permissions.</Dialog.Description>
          <div className="mt-4 space-y-4">
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Name</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. CI/CD pipeline" className="w-full rounded-md border border-border bg-transparent px-3 py-1.5 text-sm outline-none focus:border-primary/50" autoFocus />
            </div>
            <div>
              <label className="mb-1 block text-xs text-muted-foreground">Project</label>
              <Select value={selectedProjectId} onValueChange={setSelectedProjectId} options={projectOptions} className="w-full h-8 text-xs" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs text-muted-foreground">Scopes</label>
              <div className="space-y-1.5">
                {SCOPES.map((scope) => (
                  <label key={scope.value} className="flex cursor-pointer items-center gap-2.5 rounded-md border border-border px-3 py-2 transition-colors hover:bg-muted/30">
                    <input type="checkbox" checked={scopes.has(scope.value)} onChange={() => toggleScope(scope.value)} className="h-3.5 w-3.5 rounded border-border accent-primary" />
                    <div>
            ...
            <div>
              ...
              <DateTimePicker value={expiresAt} onChange={setExpiresAt} minDate={new Date()} placeholder="No expiration" className="h-8 text-xs" />
            </div>
          </div>
...
function KeyCreatedDialog({ apiKey, onClose }: { apiKey: ApiKeyCreated | null; onClose: () => void }) {
    ...
    <Dialog.Root open={!!apiKey} onOpenChange={handleOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[480px] -translate-x-1/2 -translate-y-1/2 rounded-md border border-border bg-card p-6 shadow-lg">
          ...
          <Dialog.Description className="mt-1 text-sm text-muted-foreground">Copy your key now — it won't be shown again.</Dialog.Description>
          {apiKey && (
            <div className="mt-4">
              <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-3 py-2">
                <code className="flex-1 select-all break-all text-xs">{apiKey.key}</code>
                <Button variant="ghost" size="sm" className="h-7 shrink-0 gap-1.5 text-xs" onClick={handleCopy}>
                  {copied ? <><Check className="h-3 w-3" />Copied</> : <><Copy className="h-3 w-3" />Copy</>}
                </Button>
              </div>
              <p className="mt-2 text-[10px] text-amber-500">Store this key securely. You will not be able to see it again.</p>
            </div>
          )}
          <div className="mt-5 flex justify-end">
            <Dialog.Close asChild><Button size="sm">Done</Button></Dialog.Close>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
