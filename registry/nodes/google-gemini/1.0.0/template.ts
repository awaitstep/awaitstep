export default async function (ctx) {
  ...
  const model = ctx.config.model ?? 'gemini-2.0-flash'
  ...
  switch (action) {
    case 'Generate Content': {
      ...
      if (ctx.config.temperature != null) generationConfig.temperature = ctx.config.temperature
      if (ctx.config.maxOutputTokens) generationConfig.maxOutputTokens = ctx.config.maxOutputTokens
